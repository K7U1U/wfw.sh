...
echo -e "\033[1mTotal Points d'Accès : \033[0;31m`grep -E '([A-Za-z0-9._: @\(\)\\=\[\{\}\"%;-]+,){14}' $FILE | wc -l`\e[0m"
echo -e "\033[1mTotal Stations : \033[0;31m`grep -E '([A-Za-z0-9._: @\(\)\\=\[\{\}\"%;-]+,){5} ([A-Z0-9:]{17})|(not associated)' $FILE | wc -l`\e[0m"
echo -e "\033[1mTotal Stations sans association : \033[0;31m`grep -E '(not associated)' $FILE | wc -l`\e[0m"
...
while read -r line ; do
    ...
    if [ "`echo "$line" | cut -d ',' -f 14`" != " " ]; then
        ...
        echo -e " \e[3mNe peut pas déterminer nom du réseau (ESSID)\e[0m"
    fi
    ...
    while read -r line2 ; do
        ...
        if [ "$clientsMAC" ]; then
            ...
            if [ "$result2" ]; then
                ...
                if [ "$ismobile" ]; then
                    echo -e "\t\t\t\033[0;33mC'est possiblement un mobile\e[0m"
                fi
